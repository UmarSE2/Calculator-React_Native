...
export default function App() {
    ...
    const Btn = ({ title, type }) => (
        <TouchableOpacity onPress={() => calculate(title)}
            style={[styles.btn, { backgroundColor: getColor(colors.light1, colors.dark2) }]}>
            <Text style={[styles.btnTxt, { color: getBtnColor(type) }]}>{title}</Text>
        </TouchableOpacity>
    );
    ...
    return (
        <View
            ...
            <Text style={[styles.result, { color: getColor(colors.dark, colors.light) }]}>{result}</Text>
            <View style={[styles.btnContainer, { backgroundColor: getColor(colors.light1, colors.dark1) }]}>
